...
// scripts/backfill-locations.mjs's original Locations rollout to the task
// CATALOG/configuration layer (TaskList/Task/TaskDefinition/NfcTag/
// PendingNfcLink), which shipped company-wide-only and is now becoming
// location-owned — see CLAUDE.md's "Locations" section and
// docs/features/locations.md's "Known gaps".
//
//   node --env-file=.env.local scripts/backfill-task-catalog-locations.mjs
//
...
// app ships code that relies on TaskList/Task/TaskDefinition/NfcTag's new
...
// is enough.
//
// Migration decision (confirmed with the product owner): every company's
// EXISTING shared TaskList/Task/TaskDefinition/NfcTag/PendingNfcLink data
// is assigned to that company's PRIMARY location only (its "Main Location",
// or an owner/manager's own locationId as a fallback). Any OTHER existing
// active Location under that company gets NOTHING carried over — it lazy-
// seeds a fresh set of default lists the first time anyone visits its Tasks
// page (lib/seed.ts, re-keyed to locationId), exactly like a brand-new
// Location created after this migration runs. This script deliberately
// does not clone or touch data for any location other than the primary.
//
...
if (!uri) {
  console.error("MONGODB_URI is not set. Run with: node --env-file=.env.local scripts/backfill-task-catalog-locations.mjs");
  process.exit(1);
}
...
const pendingNfcLinks = db.collection("pendingnfclinks");
...
// is skipped entirely — nothing to backfill onto; its lists will lazy-seed
// fresh at whichever location a user first visits.
...
// ObjectId. NfcTag naturally excludes still-unclaimed tags via its own
// companyId: null, which never matches the {companyId, locationId: null}
// filter below.
...
await backfillCollection(taskLists, "TaskList");
await backfillCollection(tasks, "Task");
await backfillCollection(taskDefinitions, "TaskDefinition");
await backfillCollection(nfcTags, "NfcTag");
await backfillCollection(pendingNfcLinks, "PendingNfcLink");

console.log("Done. Safe to deploy app code that relies on TaskList/Task/TaskDefinition/NfcTag/PendingNfcLink's new locationId scoping.");
await client.close();
