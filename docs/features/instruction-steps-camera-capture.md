...
Uses `Camera.getPhoto({ source: CameraSource.Camera, resultType: CameraResultType.Base64, quality: 80, width: 1600, correctOrientation: true })` — `CameraSource.Camera` is what makes this camera-only (no "choose from library" fallback), matching the decision to replace the file picker rather than add a second option. `getPhoto` is the older of `@capacitor/camera` v8's two capture APIs (v8.1 added a `takePhoto`/`MediaResult` pair meant to eventually replace it) but was kept deliberately: it's the one with a stable, documented single-dimension resize (`width` alone, aspect ratio preserved automatically), where `takePhoto`'s replacement `targetWidth`/`targetHeight` pair's single-dimension behavior isn't clearly documented. `width: 1600`/`quality: 80` is a starting point (a reference photo of a clean kitchen doesn't need full sensor resolution) — worth a quick real-device check to confirm output file size lands comfortably under the server cap (see below) across a couple of phone models, not just the simulator.

**`resultType` is `Base64`, not `Uri` — this was a real, on-device bug, not a style choice.** An earlier version used `CameraResultType.Uri` and converted the result via `fetch(photo.webPath).then(r => r.blob())`. That works fine for the Capacitor default (content served from the local `capacitor://localhost` webview), but this app instead loads its web content from a remote origin (`capacitor.config.ts`'s `server.url: 'https://chrps.vercel.app'`) — and a live `https:` page calling `fetch()` on a local `file:` resource is blocked as mixed content. `@capacitor/camera`'s own `webPath` doc comment only ever promised it's usable for "the `src` attribute of an image" (the WebView's native resource loader, a different code path than the page's own `fetch()`) — it was never actually documented as fetchable. Confirmed on a real device: capture appeared to work (Capacitor's own bridge log showed the native result crossing over to JS), but the `fetch()` call silently rejected and — since nothing caught it — left the caller's busy state hung forever ("Opening camera…" that never resolves), with the photo never actually attached. Switching to `Base64` means the image comes back as plain data already on the JS side (`photo.base64String`), decoded locally into a `File` (`atob` → `Uint8Array` → `new File([bytes], ...)`), with no second network read and no dependency on how the webview happens to be loaded.

The base64 string is **wrapped in a `File`** rather than left as a raw string or `Blob` — the calling code's existing validation (`file.type`, `file.size`) and Blob-pathname-from-filename logic were written for the old `<input type="file">` flow's `File` objects, and a bare `Blob`/string has no `.name` those needed. Returning a `File` meant zero changes to that downstream logic beyond swapping what produces it. `file.size` after decoding reflects the actual image byte size, not the (~33% larger) base64 string length, so the existing 8MB size check is unaffected by this change.

**Result handling, not exceptions**: `capturePhoto()` never throws — permission-denied and user-cancel are both promise rejections from `Camera.getPhoto()` on iOS, distinguished only by matching the rejection's message text (`/denied|permission/i` vs `/cancel/i`, since Capacitor doesn't expose a typed error code for either); the base64-decode step is wrapped in its own `try/catch` too, for the same reason the `fetch()`-based version's *lack* of one caused the hang above — a rejection/throw anywhere in this function must resolve to a typed result, never propagate uncaught. Returning a discriminated result instead of re-throwing keeps that fragile string-matching contained to one place rather than leaking into every call site's `catch` block.
...
`ios/App/App/Info.plist` also needs **both** `NSPhotoLibraryAddUsageDescription` and `NSPhotoLibraryUsageDescription` entries, even though `capturePhoto()` always calls `getPhoto` with `source: CameraSource.Camera` and this app never reads from or writes to the photo library today. This isn't a guess — `@capacitor/camera`'s native `CameraPlugin.getPhoto` (`node_modules/@capacitor/camera/ios/Sources/CameraPlugin/CameraPlugin.swift`'s `checkUsageDescriptions()`) unconditionally loops over **all three** `CameraPropertyListKeys` (`NSCameraUsageDescription`, `NSPhotoLibraryAddUsageDescription`, `NSPhotoLibraryUsageDescription`) and rejects the call if *any* is missing from `Info.plist` — it does this before even looking at `settings.source`, so which source a given call actually requests is irrelevant to this check. Missing either photo-library key surfaces as a runtime rejection on-device (`CAPCameraPlugin - You are missing <key> in your Info.plist file. Camera will not function without it.`), not a build-time warning, so a `next build`/`tsc` pass — or even an Xcode build — won't catch a missing one; only actually tapping the camera button on a real device will. The two entries:

- `NSPhotoLibraryAddUsageDescription` — *"Ch'rps may save a copy of a captured task photo to your photo library."*
- `NSPhotoLibraryUsageDescription` — *"Ch'rps may access your photo library if you choose a task photo instead of using the camera."*

Both are aspirational text for permission dialogs this app doesn't currently trigger (no `saveToGallery`, no gallery-picker source) — worth revisiting the wording if either capability is ever actually added.
