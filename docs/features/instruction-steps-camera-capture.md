...
`ios/App/App/Info.plist` also needs an `NSPhotoLibraryAddUsageDescription` entry — *"Ch'rps may save a copy of a captured task photo to your photo library."* — even though `capturePhoto()` never sets `saveToGallery` and this app never actually writes to the photo library today: `@capacitor/camera`'s compiled binary links against `Photos`/`PhotosUI`, and iOS refuses to launch the camera at all (not just refuses the specific save) if the app's Info.plist is missing a usage string for every permission its linked frameworks *could* trigger, regardless of whether the app's own code path exercises it. Xcode's own build/archive validation flags this specifically (`"You are missing NSPhotoLibraryAddUsageDescription in your Info.plist file. Camera will not function without it."`) — worth revisiting this description if `saveToGallery` (or a "save to camera roll" feature) is ever actually added, since right now it's aspirational text for a permission dialog the app doesn't currently trigger.
