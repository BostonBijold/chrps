...
**Sets up for** (not yet built): the employee-side "must attach a photo to mark this task done" feature — same Blob-upload mechanics and the same `lib/client/capture-image.ts` capture helper (see [`instruction-steps-camera-capture.md`](instruction-steps-camera-capture.md)), plus a `TaskLog`-level `photoUrl` field and a manager review surface to compare the two photos side-by-side. Deliberately built second so the storage/upload/capture plumbing only needs to be solved once.
